...
function SellAdvice() {
  ...
  const handlePredict = async (selectedStock) => {
    ...
    try {
      ...
      if (response.ok) {
        setPrediction(data);
        const fetchedName =
          data?.company_name || data?.companyName || data?.name;
  ...
  const handleAnalyze = async () => {
    ...
    navigate("/selladvice", {
      state: { symbol: sym.toUpperCase(), name: nm || undefined },
    });
  };
  ...
  useEffect(() => {
    ...
    const loadSimilar = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${backend_url}/market/similar/${encodeURIComponent(symbol)}`,
          {
            method: "GET",
            credentials: "include",
            headers: { Accept: "application/json" },
          ...
          <div>
            <div className="text-center p-8 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 mb-10">
              ...
              <p className="text-[#4c809a] dark:text-slate-400 max-w-xl mx-auto mb-8">
                Sign in to access comprehensive AI-powered stock risk analysis
                and get detailed insights for {symbol || "your stocks"}.
              </p>
              <div className="flex gap-3 justify-center">
                <Link
                  to="/login"
                  className="inline-flex items-center justify-center rounded-xl bg-[#13a4ec] hover:bg-[#0f8ac4] px-6 py-3 text-base font-bold text-white shadow-md shadow-[#13a4ec]/20 transition-all"
                >
                  Login
                </Link>
                <Link
                  to="/register"
                  className="inline-flex items-center justify-center rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 px-6 py-3 text-base font-bold text-[#13a4ec] ring-2 ring-inset ring-[#13a4ec] transition-all"
                >
                  Sign Up
                </Link>
              </div>
              ...
              <div className="flex flex-col gap-6">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl ring-1 ring-slate-200 dark:ring-slate-700">
                    <div className="flex flex-col md:flex-row justify-between items-start mb-5">
                      ...
                      {prediction.price_change > 0 ? (
                        <div className="text-xs md:text-xl font-bold text-green-600">
                          {prediction.price_change}
                        </div>
                      ) : (
                        <div className="text-xs md:text-xl font-bold text-red-600">
                          {prediction.price_change}
                        </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-5 rounded-lg ring-1 ring-slate-200 dark:ring-slate-700 mb-6">
                      <h4 className="font-bold text-[#0d171b] dark:text-white mb-2">
                        Recommendation
                      </h4>
                    ...
                    <div>
                      <h4 className="font-bold text-[#0d171b] dark:text-white mb-3">
                        Why this assessment
                      </h4>
                  ...
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-xl ring-1 ring-slate-200 dark:ring-slate-700 flex flex-col items-center justify-center gap-4">
                    <div
                      className={`text-base font-bold px-4 py-2 rounded-full ${prediction.risk_level === "HIGH" ? "bg-red-500/15 text-red-500" : prediction.risk_level === "MEDIUM" ? "bg-amber-500/15 text-amber-500" : "bg-green-500/15 text-green-500"}`}
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-xl ring-1 ring-slate-200 dark:ring-slate-700">
                    <h4 className="font-bold text-[#0d171b] dark:text-white mb-4">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl ring-1 ring-slate-200 dark:ring-slate-700">
                  <div className="flex items-center justify-between gap-4 mb-4">
                    <h4 className="font-bold text-[#0d171b] dark:text-white">
                      Similar stocks
                    </h4>
                    {similarLoading && (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      {(similarStocks || []).map((s) => (
                        <button
                          ...
                          onClick={() =>
                            navigate("/selladvice", {
                              state: { symbol: s.symbol, name: s.name },
                            })
                          }
